...
另有一张独立卡片长按显示/隐藏关键点叠加层，左侧显示当前模型介绍卡。

## Face Landmarker 3D 头盔原型

Face Landmarker 模式新增第一阶段 3D 科幻头盔 POC：

1. 开启摄像头，通过右侧卡片切换到“人脸网格 / Face Landmarker”。
2. 点击左侧“装备头盔”按钮，程序生成的分块头盔会装配到第一张检测到的人脸上；再次点击可反向卸下。
3. “显示/隐藏捕捉关键点”仍只控制 2D 面部网格，和 3D 头盔互不影响。

头盔完全由 Three.js 基础几何体和材质在运行时生成，不是最终 GLB/FBX 素材。它用于验证镜像/contain 映射、位置与远近缩放、三轴旋转、平滑和装配状态机；不包含正式遮挡、电影级光照或最终资产。浏览器需要 WebGL；如果 WebGL 初始化失败，页面会提示降级，其他模型演示仍会尽可能继续工作。
...
- 头盔按钮是普通点击控件，支持鼠标、触摸和键盘；第一阶段不使用眨眼、张嘴或手势自动触发
